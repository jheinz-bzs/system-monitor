...
**Date:** 2026-06-24
**Status:** Active — amended 2026-07-09: the bare Linux binary now also self-updates when its install directory is writable. `assetName()` resolves the `.AppImage` only for AppImage launches (`$APPIMAGE` set) and the extensionless binary otherwise; `update.Supported()` gates a bare-binary launch on a writability probe of the executable's directory (create+remove a temp file — the exact operation the installer performs), so a bare binary outside `/usr` self-updates only when its install dir is writable. The "no dishonest UI" rationale below is preserved by the probe rather than by excluding bare binaries wholesale. — amended 2026-08-03 (issue #68): a dpkg-owned install (the `.deb` under `/usr/bin`) now **does** show an update affordance, but in deferred mode — `update.InstallMode()` classifies it `ModePackageManager`, so the banner and status-bar pill appear with an "Update via apt" link that surfaces `sudo apt update && sudo apt upgrade` (the apt repo, #66, is live) instead of self-updating, and the controller's `Start` is a no-op in that mode so an auto-install preference can never swap the file out from under dpkg's database.
...
**The Linux release publishes three artifacts from one native build: a bare binary (`system-monitor-linux-amd64`, manual/server use), a `.deb` (apt-managed), and an AppImage (`system-monitor-linux-amd64.AppImage`). In-app self-update on Linux targets the AppImage and a writable bare-binary dir: `assetName()` returns the `.AppImage` for AppImage launches, and swap/restart/download operate on `$APPIMAGE` (the running AppImage file) rather than `os.Executable()` (which points into the read-only mount). `update.Supported()` wires the updater for AppImage and writable bare-binary launches, and — since issue #68 — a dpkg-owned `.deb` install too, but in deferred mode: the banner/status-bar pill appear with an "Update via apt" link that defers to the package manager instead of swapping the binary, while an unwritable, non-distro install (e.g. root-owned `/opt`) still shows no affordance. The `.deb` is built with nfpm and the AppImage with linuxdeploy in the release workflow.**

### Rationale

One native build feeds all three packagers, so there's no extra compile. Making the AppImage the self-update target is the natural fit — it's already a relocatable, user-writable single file, so the existing rename-swap works unchanged once it targets `$APPIMAGE` (a one-line indirection via `targetExecutable()`, shared by swap, restart, and the download dir so the temp file lands on the AppImage's volume for an atomic rename). Gating with `Supported()` avoids dishonest UI: a `.deb` user would otherwise see "update failed" when the swap hit a permission wall, so instead the affordance points at apt with the one-shot upgrade command (issue #68) — the correct distro convention — and an install with no update path at all (a root-owned non-distro dir) shows nothing rather than a doomed "Update". The dpkg-owned detection is path-based (`/usr/bin/` prefix, the nfpm install target) rather than `dpkg -S` because it needs no subprocess, works on any distro, and matches exactly where the .deb lands; a binary hand-placed in another root-owned dir keeps today's no-affordance behavior. nfpm (a Go binary, `go install`-able on the runner) drives the `.deb` from a small YAML with no FPM/Ruby; linuxdeploy bundles the GL/X11 shared libs the Fyne binary links, so the AppImage runs on machines without them. macOS code-signing/notarization remains deferred (ADR-009); this ADR only broadens Linux.
